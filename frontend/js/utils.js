...
const Utils = {
  ...
  renderAvatar(name, imageUrl, size = 56) {
    ...
    if (imageUrl) {
      return `<div class="avatar" style="width:${size}px;height:${size}px;background:${bgColor}">
                <img src="${imageUrl}" alt="${name}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                <div style="display:none;width:100%;height:100%;align-items:center;justify-content:center;font-weight:700;font-size:${size/2}px">${initials}</div>
              </div>`;
    }
    return `<div class="avatar" style="width:${size}px;height:${size}px;background:${bgColor};font-size:${size/2}px">${initials}</div>`;
  },
