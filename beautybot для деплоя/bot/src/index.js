// Try to load .env from the path specified by server.js, fallback to backend/.env
const dotenvPath = process.env.DOTENV_PATH || require('path').join(__dirname, '..', 'backend', '.env');
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('confirm_')) {
    ...
    try {
      ...
      await axios.put(`http://localhost:${BACKEND_PORT}/api/bookings/${bookingId}/status`, {
        status: 'confirmed'
      }, {
