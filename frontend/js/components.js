    ...
    return `
      <div class="service-card ${selected ? 'selected' : ''} stagger-item" onclick="${onClick}">
        ...
        <div class="service-info">
          ...
          <div class="service-meta">⏱ ${duration}</div>
        </div>
        <div class="service-price">${price}</div>
        ${selected ? '<div style="color:var(--color-primary);margin-left:4px;font-weight:700">✓</div>' : ''}
      </div>
    `;
  }
};
  ...
  render(master, options = {}) {
    ...
    return `
      <div class="master-card ${selected ? 'selected' : ''} stagger-item" onclick="${onClick}">
        ...
        ${selected ? '<div style="color:var(--color-primary);font-size:20px;font-weight:700">✓</div>' : `
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-text-tertiary)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M9 18l6-6-6-6"/>
          </svg>
        `}
      </div>
    ...
    return `
      <div class="booking-card stagger-item" onclick="${onClick}">
        ...
        <div class="booking-card-body">
          ...
          ${booking.price ? `<div style="font-size:var(--font-size-sm);color:var(--color-primary-dark);font-weight:600;margin-top:4px">${Utils.formatPrice(booking.price)}</div>` : ''}
        </div>
      </div>
    `;
  }
...
const ConfirmDialog = {
  open(title, text, onConfirm, options = {}) {
    const { confirmText = 'Подтвердить', cancelText = 'Отмена', icon = '⚠️', variant = 'danger' } = options;
    ...
    overlay.innerHTML = `
      <div class="confirm-dialog">
        ...
        <div class="confirm-dialog-actions">
          ...
          <button class="btn ${variant === 'danger' ? 'btn-danger' : 'btn-primary'}" id="confirm-dialog-ok">${confirmText}</button>
        </div>
      </div>
    `;
