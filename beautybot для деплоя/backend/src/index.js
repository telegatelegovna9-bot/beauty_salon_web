...
// Use process.cwd() to get the project root (where server.js is run from)
const PROJECT_ROOT = process.cwd();
const FRONTEND_PATH = path.join(PROJECT_ROOT, 'frontend');
const DB_PATH_ENV = process.env.DB_PATH || './backend/data/beauty_salon.db';
const DB_PATH = path.resolve(PROJECT_ROOT, DB_PATH_ENV);
const UPLOADS_PATH_ENV = process.env.UPLOADS_PATH || './backend/uploads';
const UPLOADS_PATH = path.resolve(PROJECT_ROOT, UPLOADS_PATH_ENV);
...
if (process.env.NODE_ENV === 'production') {
  if (fs.existsSync(FRONTEND_PATH)) {
    ...
    console.warn(`⚠️ Frontend directory not found at: ${FRONTEND_PATH}`);
  }
}
...
if (process.env.NODE_ENV === 'production') {
  ...
  console.log(`📁 Frontend exists: ${require('fs').existsSync(FRONTEND_PATH)}`);
